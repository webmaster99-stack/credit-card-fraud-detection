...
if __name__ == "__main__":
    ...
    transactions = generate_transactions(n_samples=10000, fraud_ratio=0.02)
    
    # Split into train (80%) and test (20%)
    train_df = transactions.sample(frac=0.8, random_state=42)
    test_df = transactions.drop(train_df.index)
    
    # Save to CSV files
    train_df.to_csv("notebooks/data/train.csv", index=False)
    test_df.to_csv("notebooks/data/test.csv", index=False)
    
    # Print summary statistics
    print(f"\nDataset generated successfully!")
    print(f"Training set: {len(train_df):,} transactions")
    print(f"Test set: {len(test_df):,} transactions")
    print(f"Overall fraud ratio: {transactions['is_fraud'].mean():.2%}")
    print(f"\nLegitimate transactions - Average amount: ${transactions[transactions['is_fraud']==0]['amount'].mean():.2f}")
    print(f"Fraudulent transactions - Average amount: ${transactions[transactions['is_fraud']==1]['amount'].mean():.2f}")
    print(f"\nMerchant category distribution (fraud):")
    print(transactions[transactions['is_fraud'] == 1]['merchant_category'].value_counts(normalize=True))